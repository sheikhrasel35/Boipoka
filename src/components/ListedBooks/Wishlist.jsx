    ...
    <div className="max-w-[1170px] mx-auto space-y-4 p-4">
      {wishlist.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 mt-[80px] mb-[80px]">
          ...
          <h1 className="text-4xl font-semibold mt-4 mb-4">Oops! No Book found!</h1>
          <p className="text-[#627382] text-lg mb-6">
            You haven’t added any books to your Wishlist yet. Start exploring and mark your favorites!
          </p>
        </div>
      ) : (
        wishlist.map((book) => <Booklist key={book.bookId} book={book} />)
      )}
    </div>
  );
};
